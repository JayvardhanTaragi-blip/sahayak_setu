...
window.API = (function () {
  ...
  const maps = {
    ...
    // Same endpoint, full payload — { workers, center, count }. `workers` above
    // is kept because existing callers expect a bare array.
    pins: (params) => get('/api/maps/workers' + qs(params || {})),
  ...
  return { BASE, session, auth, services, workers, bookings, admin, chat, maps, get, post, patch, health: () => get('/api/health') };
})();
