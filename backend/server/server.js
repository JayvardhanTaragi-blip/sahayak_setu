...
app.get('/api/health', (_req, res) => {
  ...
  const demo = db
    ...
    .get().n === 3;
  res.json({ ok: true, ai: ai.hasKey() ? 'gemini' : 'fallback', demo, counts });
});
...
app.listen(PORT, () => {
  ...
  AI mode     ${ai.hasKey() ? `Gemini (${process.env.GEMINI_MODEL || 'gemini-2.5-flash'})` : 'offline fallback — add GEMINI_API_KEY to .env for real AI'}
