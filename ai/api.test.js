...
(async () => {
  ...
  ok('health reports fallback AI or gemini', ['fallback', 'gemini'].includes(r.json && r.json.ai), JSON.stringify(r.json && r.json.ai));
  ...
  ok('chat reports valid source', ['fallback', 'gemini'].includes(r.json && r.json.source), JSON.stringify(r.json && r.json.source));
